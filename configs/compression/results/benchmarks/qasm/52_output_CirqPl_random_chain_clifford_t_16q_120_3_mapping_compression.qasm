// Copyright (c) 2019 Turation Ltd

OPENQASM 2.0;
include "qelib1.inc";

qreg q[16];
creg c[16];
u3(-1*pi,1.9634954084936207,-1.9634954084936207) q[1];
u3(-1*pi,2.748893571891069,-2.748893571891069) q[3];
u2(-1*pi,1*pi) q[4];
u3(-1*pi,2.748893571891069,-2.748893571891069) q[0];
u2(-1*pi,1*pi) q[10];
u3(-1*pi,2.748893571891069,-2.748893571891069) q[11];
u2(-1*pi,1*pi) q[12];
u3(-1*pi,2.748893571891069,-2.748893571891069) q[13];
u2(-1*pi,1*pi) q[6];
u3(-1*pi,2.748893571891069,-2.748893571891069) q[2];
u2(-1*pi,1*pi) q[15];
u2(-1*pi,1*pi) q[14];
u2(-1*pi,1*pi) q[5];
u3(-1*pi,2.748893571891069,-2.748893571891069) q[7];
u2(-1*pi,1*pi) q[8];
u2(-1*pi,1*pi) q[9];
cz q[11], q[12];
cz q[13], q[6];
cz q[2], q[15];
cz q[0], q[10];
cz q[3], q[4];
cz q[11], q[14];
cz q[1], q[12];
u2(-1.25*pi,1.25*pi) q[2];
u3(-0.25*pi,0.50*pi,-0.50*pi) q[15];
u3(-0.25*pi,0.50*pi,-0.50*pi) q[10];
cz q[11], q[6];
u2(-0.50*pi,0.50*pi) q[14];
u2(-0.25*pi,0.25*pi) q[1];
u2(-1*pi,1*pi) q[12];
cz q[11], q[5];
cz q[7], q[14];
u2(-1*pi,1*pi) q[6];
cz q[3], q[14];
u2(0,0) q[5];
u2(-0.25*pi,0.25*pi) q[7];
cz q[0], q[7];
u2(-0.25*pi,0.25*pi) q[3];
cz q[11], q[3];
u2(-0.25*pi,0.25*pi) q[0];
u2(-0.25*pi,0.25*pi) q[7];
cz q[13], q[0];
cz q[7], q[15];
u2(-0.25*pi,0.25*pi) q[11];
u2(-0.25*pi,0.25*pi) q[3];
cz q[3], q[11];
u2(-0.25*pi,0.25*pi) q[0];
u2(-1*pi,1*pi) q[15];
cz q[0], q[8];
u2(-1.25*pi,1.25*pi) q[3];
cz q[15], q[8];
u2(-1.25*pi,1.25*pi) q[0];
cz q[6], q[0];
cz q[15], q[3];
u2(-1*pi,1*pi) q[8];
cz q[7], q[0];
cz q[8], q[9];
u2(-1.25*pi,1.25*pi) q[3];
u2(-0.75*pi,0.75*pi) q[7];
u3(-0.25*pi,0.25*pi,-0.25*pi) q[0];
u2(0,0) q[8];
u2(0,0) q[9];
cz q[5], q[8];
cz q[9], q[4];
cz q[6], q[8];
cz q[9], q[10];
u2(-1*pi,1*pi) q[5];
u2(-1*pi,1*pi) q[4];
cz q[4], q[5];
cz q[13], q[8];
cz q[9], q[11];
cz q[15], q[8];
u2(0,0) q[4];
u3(-1*pi,1*pi,-1*pi) q[5];
u3(-0.25*pi,0.25*pi,-0.25*pi) q[11];
cz q[2], q[4];
cz q[5], q[0];
cz q[13], q[11];
u2(0,0) q[15];
u2(-1.50*pi,1.50*pi) q[2];
u2(0,0) q[4];
u2(-1*pi,1*pi) q[5];
u2(-1.25*pi,1.25*pi) q[0];
cz q[4], q[8];
cz q[12], q[5];
cz q[0], q[14];
cz q[4], q[10];
u2(0,0) q[12];
u2(-1*pi,1*pi) q[5];
u2(0,0) q[0];
u2(-1*pi,1*pi) q[14];
cz q[5], q[1];
cz q[13], q[12];
cz q[14], q[11];
u2(-1*pi,1*pi) q[4];
cz q[6], q[4];
u2(0,0) q[5];
u2(-0.25*pi,0.25*pi) q[1];
u2(0.25*pi,-0.25*pi) q[13];
u2(-1*pi,1*pi) q[14];
u2(-1.25*pi,1.25*pi) q[11];
cz q[1], q[7];
cz q[9], q[13];
rz(1.3107401488595243) q[14];
u2(-0.50*pi,0.50*pi) q[6];
u2(-1*pi,1*pi) q[4];
cz q[3], q[6];
u2(-1*pi,1*pi) q[9];
u2(0.25*pi,-0.25*pi) q[13];
cz q[13], q[15];
cz q[4], q[9];
u2(-0.25*pi,0.25*pi) q[3];
u2(0,0) q[6];
cz q[11], q[3];
cz q[13], q[0];
u2(0,0) q[15];
u2(0,0) q[4];
u2(-1*pi,1*pi) q[9];
cz q[15], q[7];
cz q[9], q[12];
u2(-0.25*pi,0.25*pi) q[11];
u2(-0.25*pi,0.25*pi) q[3];
u2(-0.25*pi,0.25*pi) q[13];
cz q[3], q[6];
cz q[9], q[11];
u2(-1*pi,1*pi) q[15];
u2(-0.75*pi,0.75*pi) q[7];
u2(-1*pi,1*pi) q[12];
cz q[12], q[8];
rz(-1*pi) q[7];
u2(0.50*pi,-0.50*pi) q[6];
u2(0,0) q[9];
u2(-0.25*pi,0.25*pi) q[11];
cz q[6], q[13];
u2(0,0) q[12];
u2(-1*pi,1*pi) q[8];
cz q[8], q[5];
cz q[6], q[15];
u2(-0.25*pi,0.25*pi) q[13];
cz q[8], q[10];
u2(0,0) q[5];
u2(-0.50*pi,0.50*pi) q[6];
cz q[5], q[4];
u2(-1*pi,1*pi) q[8];
u3(-1.0471975510918778,2.5261129449674837,-2.5261129449674837) q[10];
cz q[13], q[4];
cz q[8], q[15];
u2(0.50*pi,-0.50*pi) q[5];
cz q[1], q[5];
u3(-1*pi,1.1780972450961724,-1.1780972450961724) q[13];
u2(0,0) q[4];
u2(-1*pi,1*pi) q[8];
u2(0,0) q[15];
cz q[15], q[8];
u2(0.50*pi,-0.50*pi) q[1];
u2(0.50*pi,-0.50*pi) q[5];
cz q[5], q[6];
rz(1.3948088512828882) q[1];
u2(-1*pi,1*pi) q[8];
u2(-0.50*pi,0.50*pi) q[5];
u2(-0.50*pi,0.50*pi) q[6];
cz q[6], q[9];
cz q[3], q[5];
u2(0.50*pi,-0.50*pi) q[6];
u2(-1*pi,1*pi) q[9];
u3(-1*pi,0.25*pi,-0.25*pi) q[3];
u2(-0.50*pi,0.50*pi) q[5];
cz q[14], q[1];
rz(-1*pi) q[9];
u2(-2.616250668127717,2.616250668127717) q[14];
u2(1.3948088512828882,-1.3948088512828882) q[1];
rz(2.616250668127717) q[14];
rz(0.17598747551200827) q[1];
cz q[14], q[0];
cz q[15], q[1];
cz q[15], q[12];
u2(0,0) q[14];
u2(-1*pi,1*pi) q[0];
u2(-1*pi,1*pi) q[1];
cz q[0], q[2];
rz(-1*pi) q[14];
rz(-1*pi) q[1];
u3(-1*pi,0.50*pi,-0.50*pi) q[15];
u2(0,0) q[12];
cz q[11], q[2];
cz q[12], q[6];
u3(-1*pi,0,0) q[0];
u3(-1*pi,1.25*pi,-1.25*pi) q[11];
u2(-0.50*pi,0.50*pi) q[2];
u3(-1*pi,-0.25*pi,0.25*pi) q[12];
u2(-0.50*pi,0.50*pi) q[6];
cz q[8], q[11];
cz q[2], q[10];
s q[6];
u2(0,0) q[8];
u2(-0.25*pi,0.25*pi) q[11];
u3(-1*pi,0.25*pi,-0.25*pi) q[2];
u2(-1.230959417244619,1.230959417244619) q[10];
cz q[5], q[8];
rz(0.25*pi) q[11];
rz(1.230959417244619) q[10];
u3(-1*pi,0.25*pi,-0.25*pi) q[5];
u2(-1*pi,1*pi) q[8];
rz(-1*pi) q[8];
